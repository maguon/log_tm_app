...
export default class DriverListItem extends Component {
    ...
    render() {
        if (this.props.initParam.license_date) {
            ...
            // &&(Date.parse(new Date(this.props.initParam.license_date)))>(Date.parse(new Date()))
        }
            ...
            <TouchableNativeFeedback onPress={this.props.onPress} background={TouchableNativeFeedback.SelectableBackground()}>
                <View style={{ flexDirection: 'row', backgroundColor: '#fff', borderWidth: 0.5, borderColor: '#e6ebee', borderRadius: 1, marginTop: 10 }}>
                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        {this.props.initParam.operate_type == 2 && <FontTag size={40} title='协' color='#73de8a' fontColor='#fff' />}
                        {this.props.initParam.operate_type == 3 && <FontTag size={40} title='供' color='#efbb7a' fontColor='#fff' />}
                        {this.props.initParam.operate_type == 4 && <FontTag size={40} title='包' color='#e08ddd' fontColor='#fff' />}
                    </View>
                    <View style={{ flex: 2 }} >
                    ...
                    <View style={{ flex: 2, justifyContent: 'flex-end' }} >
                        <View style={{ marginVertical: 8, alignItems: 'center' }}>
                            {this.props.initParam.gender == '男' && <Image source={{ uri: 'man' }} style={{ width: 6, height: 12 }} />}
                            {this.props.initParam.gender == '女' && <Image source={{ uri: 'woman' }} style={{ width: 6, height: 12 }} />}
                        </View>
