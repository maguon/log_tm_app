...
class OveruseDieselOilToolButton extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    render() {
        return (
            <View style={{ flexDirection: 'row' }}>
                ...
                <MenuForHeader
                    menuList={[
                        { icon: () => <Icon name='ios-add' style={{ fontSize: 20, color: '#777' }} />, title: '增加超油', route: Actions.createOveruseDieselOil },
                        { icon: () => <Icon name='ios-search' style={{ fontSize: 20, color: '#777' }} />, title: '搜索', route: Actions.overuseDieselSearch }
                    ]}
